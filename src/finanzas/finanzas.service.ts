...
export class FinanzasService {
  constructor(
    ...
    @InjectRepository(CustomKeyword)
    private customKeywordRepo: Repository<CustomKeyword>,
  ...
  async saveAllData(userId: number, data: any) {
    ...
    const notificationsToSave = notificationsArr.map((n) => ({
      ...stripId(n),
      userId,
    }));
    const recurringsToSave = recurringsArr.map((r) => ({
      ...stripId(r),
      userId,
    }));
  ...
  async saveUserTransactions(userId: number, transactions: any[]) {
    ...
    const userExists = await this.transactionRepo.manager.query(
    ...
    if (!userExists.length) {
      throw new Error(
        'El usuario no existe en la base de datos. Reautentica tu sesión.',
      );
    }
    ...
    console.log(
      `[saveUserTransactions] user=${numUserId} received ${transactions.length} items, saving ${txWithUser.length}`,
    );
  ...
  async updateRecurring(userId: number, id: number, recurring: any) {
    ...
    if (typeof recurring.executionCount === 'number') {
      ...
      const yearExecutions = recurring.executionHistory.filter(
        (date: string) => {
  ...
  async saveCustomKeywords(userId: number, payload: any) {
    ...
    if (Array.isArray(payload))
      arr = payload.map((k: any) => ({ keyword: String(k) }));
        ...
        Object.values(payload).forEach((v: any) => {
          if (Array.isArray(v)) flat.push(...v.map((x: any) => String(x)));
            ...
            Object.values(v).forEach((vv: any) => {
              if (Array.isArray(vv))
                flat.push(...vv.map((x: any) => String(x)));
            });
        });
